...
# test access
temp=$(mktemp)
$CURL localhost:9200 -w "%{http_code}" >$temp 2>/dev/null || {
  [ "$(cat $temp)" = "401" ] && echo "Kibana is protected, please provide ES_USERNAME and ES_PASSWORD env variable" && exit 1
  echo "Cannot access Kibana, HTTP error code returned: $(cat $temp)" && exit 1
}
